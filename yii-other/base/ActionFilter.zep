...
class ActionFilter extends Behavior
{
    ...
    {
        var mid,id;
        if typeof this->owner == "object" &&  (this->owner instanceof Module) {
            let mid = this->owner->getUniqueId();
            let id = action->getUniqueId();
    ...
    {
        var id,onlyMatch,exceptMatch;
        let id = this->getActionId(action);

        if empty this->only {
            let onlyMatch = true;
        } else {
            let onlyMatch = false;
            for pattern in this->only {
        ...
        for pattern in this->except {
            if fnmatch(pattern, id) {
               let exceptMatch = true;
               break;
            }
        }
