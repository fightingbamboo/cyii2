...
class Controller extends Component implements ViewContextInterface
{
    ...
    {
        var action,oldAction,runAction;
        ...
        if BaseYii::$app->requestedAction === null {
            let BaseYii::$app->requestedAction = action;
        }
        ...
        for module in this->getModules() {
            if typeof module == "object" && module->beforeAction(action) {
    ...
    public function run(string route,array params = [])
    {
        ...
        if pos === false {
            ...
            return BaseYii::$app->runAction(ltrim(route, '/'), params);
        }
    }
    ...
    {
        var layoutFile;
        let layoutFile = this->findLafindLayoutFileyoutFile(this->getView());
        if layoutFile !== false {
    ...
    {
        if this->_view === null {
            let this->_view = BaseYii::$app->getView();
        }
        return this->_view;
    }
    ...
    {
        this->_viewPath = BaseYii::getAlias(path);
    }
    ...
    public function findLayoutFile(<View> view)
    {
        ...
        if strncmp(layout, '@', 1) === 0 {
            ...
            let file = BaseYii::$app->getLayoutPath() . DIRECTORY_SEPARATOR . substr(layout, 1);
        } else {
