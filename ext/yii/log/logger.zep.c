...
PHP_METHOD(yii_log_Logger, init) {
	...
	ZEPHIR_CALL_PARENT(NULL, yii_log_logger_ce, this_ptr, "init", &_0, 27);
	...
	ZEPHIR_CALL_FUNCTION(NULL, "register_shutdown_function", NULL, 28, _1, ZEPHIR_GLOBAL(global_true));
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(yii_log_Logger, log) {
	...
	if (ZEPHIR_GT_LONG(_0, 0)) {
		count = 0;
		ZEPHIR_CALL_FUNCTION(&ts, "debug_backtrace", NULL, 29);
		zephir_check_call_status();
		ZEPHIR_MAKE_REF(ts);
		ZEPHIR_CALL_FUNCTION(NULL, "array_pop", NULL, 30, ts);
		ZEPHIR_UNREF(ts);
		zephir_check_call_status();
...
PHP_METHOD(yii_log_Logger, calculateTimings) {
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_1, &_0)
		...
		if (ZEPHIR_IS_LONG(level$$3, 0x60)) {
			...
			ZEPHIR_CALL_FUNCTION(&last$$5, "array_pop", &_3, 30, stack);
			ZEPHIR_UNREF(stack);
			zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(NULL, "ksort", NULL, 31, timings);
