...
class BaseUrl
{
    ...
    public static function toRoute(route, scheme = false) -> string
    {
        ...
        let route[0] = $static::normalizeRoute(route[0]);
        if scheme {
    ...
    {
        let route =  BaseYii::getAlias( route);
        if strncmp(route, "/", 1) === 0 {
            // absolute route
            return ltrim(route, "/");
        }
        // relative route
        if BaseYii::app->controller === null {
            throw new InvalidParamException("Unable to resolve the relative route: {route}. No active controller is available.");
        }
        if strpos(route, "/") === false {
            // empty or an action ID
            return  route === "" ? BaseYii::app->controller->getRoute()  : BaseYii::app->controller->getUniqueId() . "/" . route;
        } else {
            // relative to module
            return ltrim(BaseYii::app->controller->module->getUniqueId() . "/" . route, "/");
        }
    }
    ...
    public static function to(url = "", scheme = false) -> string
    {
        ...
        if url === "" {
            let url =  BaseYii::app->getRequest()->getUrl();
        }
        if !(scheme) {
            return url;
        }
        ...
        let pos =  strpos(url, ":");
        if pos === false || !(ctype_alpha(substr(url, 0, pos))) {
            // turn relative URL into absolute
            let url =  $static::getUrlManager()->getHostInfo() . "/" . ltrim(url, "/");
        }
        let pos =  strpos(url, ":");
        if is_string(scheme) && pos !== false {
    ...
    public static function base(scheme = false) -> string
    {
        ...
        let url =  $static::getUrlManager()->getBaseUrl();
        if scheme {
            let url =  $static::getUrlManager()->getHostInfo() . url;
            let pos =  strpos(url, "://");
            if is_string(scheme) && pos !== false {
                let url =  scheme . substr(url, pos);
            }
    ...
    {
        let url =  $static::to(url);
        if name === null {
            BaseYii::app->getUser()->setReturnUrl(url);
        } else {
            BaseYii::app->getSession()->set(name, url);
        }
    }
    ...
    {
        if name === null {
            return BaseYii::app->getUser()->getReturnUrl();
        } else {
            return BaseYii::app->getSession()->get(name);
        }
    }
    ...
    public static function canonical() -> string
    {
        ...
        let params =  BaseYii::app->controller->actionParams;
        let params[0] =  BaseYii::app->controller->getRoute();
    ...
    public static function home(scheme = false) -> string
    {
        ...
        let url =  BaseYii::app->getHomeUrl();
        if scheme {
            let url =  $static::getUrlManager()->getHostInfo() . url;
            let pos =  strpos(url, "://");
            if is_string(scheme) && pos !== false {
     ...
     */
    public static function isRelative(string url) -> boolean
    {
    ...
    {
        var currentParams, route;
    
        let currentParams =  BaseYii::app->getRequest()->getQueryParams();
        let currentParams[0] =  "/" . BaseYii::app->controller->getRoute();
        let route =  $static::merge(currentParams, params);
    ...
    {
        return  $static::urlManager ? $static::urlManager : BaseYii::app->getUrlManager();
    }
    ...
    {
        var args, res, next, k, v;

        let args =  func_get_args();
        let res =  array_shift(args);
        while (!(empty(args))) {
